...
# ---- 追加：凡例だけを画像にするユーティリティ ----
from matplotlib.lines import Line2D

from matplotlib.lines import Line2D

def save_legend_only_from_axes(ax, outpath, ncol=2, fontsize=18, dpi=300):
    """描画済み ax の凡例情報から、白背景の凡例画像を作る"""
    # 元軸からハンドルとラベルを取得（重複ラベルは除去）
    handles, labels = ax.get_legend_handles_labels()
    uniq = {}
    for h, lab in zip(handles, labels):
        if lab not in uniq:
            # 背景の線スタイルだけをコピーした「代理ハンドル」を作る
            ph = Line2D([0], [0],
                        color=h.get_color(),
    ...
    # 凡例専用の Figure を用意（白背景）
    fig = plt.figure(figsize=(4, 0.55*max(1, len(labels)/ncol)), dpi=dpi)
    fig.patch.set_facecolor("white")  # 背景を白に設定
    ax_dummy = fig.add_subplot(111)
    ax_dummy.axis("off")

    # fig.legend で中央に凡例だけ描画
    fig.legend(handles, labels,
               loc="center", frameon=False,
               ncol=ncol, fontsize=fontsize, handlelength=2.5)

    # 白背景で保存
    fig.savefig(outpath, bbox_inches="tight", pad_inches=0.05, transparent=False)
    plt.close(fig)
def make_setting_scale(dataset):
    setting = {"australian": {"train": {"x_min": 0, "x_max": 0.5, "y_min": 0.90, "y_max": 0.94},
                              "test": {"x_min": 0, "x_max": 0.5, "y_min": 0.84, "y_max": 0.94},
                              "legend_size": 15},
               ...
               "vehicle": {"train": {"x_min": 0, "x_max": 0.5, "y_min": 0.74, "y_max": 0.9},
                           "test": {"x_min": 0, "x_max": 0.5, "y_min": 0.65, "y_max": 0.76},
                           "legend_size": 13},
               ...
               "glass": {"train": {"x_min": 0, "x_max": 0.5, "y_min": 0.8, "y_max": 0.9},
                         "test": {"x_min": 0, "x_max": 0.5, "y_min": 0.64, "y_max": 0.7},
                         "legend_size": 15},
...
def make_threshold_setting():
    setting = {
        "single": {"linestyle": "dashed", "label": "ST"},
        "cwt": {"linestyle": "solid", "label": "CWT"},
        # "rwt" : {"linestyle" : "dotted", "label": "RWT"}
    }
...
class plotMLModelARC():
    ...
    def _get_threshold_base(self, setting, model, num_stage, base, y_measure):
        ...
        algorithmID = "./results/20250615/" + setting["algorithmID"]
        ...
        files = [f"{algorithmID}/{dataset}/trial{rr}{cc}/non-sorted-{y_measure}-{base}.csv" for rr in range(RR) for cc in
                 range(CC)]

        if num_stage == 2:
            files = [f"{algorithmID}/{dataset}/trial{rr}{cc}/{model}/second-{y_measure}-{base}.csv" for rr in range(RR)
                     for cc in range(CC)]
    ...
    def _get_threshold_mean(self, setting, y_measure, base):
        ...
        def step_mean(dfs, step=0.02):
            def df_one_step(t_min, t_max):
                ...
                df_model = [pd.DataFrame(np.array([list(df["accuracy"]), list(df["rejectrate"])]).T, columns=columns)
                            for df in dfs if len(df) > 3]
            ...
            return [[(df["accuracy"].min(), df["accuracy"].mean(), df["accuracy"].max()),
                     (df["rejectrate"].min(), df["rejectrate"].mean(), df["rejectrate"].max())] for df in df_mean if
                    df is not None]
    ...
    def plot_threshold_base(self, setting, y_measure, num_stage, fig, axes):
        ...
        if num_stage == 1:
            ...
            for base in bases:
                ...
                plt.plot(results["rejectrate"],
                         ...
                         label=setting_threshold[base]["label"])
        ...
        for base in bases:
            ...
            axes.plot(reject_mean,
                      ...
                      color=color,
                      linestyle=setting_threshold[base]["linestyle"],
                      label="Two-stage " + setting_threshold[base]["label"])
    ...
    def plot_ARC(self, setting, x_measures=["reject_train", "reject_test"], y_measures=["train", "test"]):
        ...
        for x_measure, y_measure in zip(x_measures, y_measures):
            ...
            ##凡例
            ##axes.legend(fontsize = 24, loc = "lower right")

            output_dir = f"./results/NSGA-II_2stage_RO/plots/{dataset}/"

            if not os.path.exists(output_dir):
                os.makedirs(output_dir)

            fig.savefig(f"{output_dir}/{dataset}_{y_measure}.png")
            fig.savefig(f"{output_dir}/{dataset}_{y_measure}.png")
            ...
            # fig.savefig(f"{output_dir}/png/{dataset}_{y_measure}.png", dpi = 300)
            legend_path = f"{output_dir}/{dataset}_legend.png"
            #save_legend_only_from_axes(axes, legend_path, ncol=2, fontsize=22, dpi=300)
        return fig, axes
